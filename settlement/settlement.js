...
const pushSettlementData = async (SettlementStream, rawData) => {
    try {
        if (!rawData) {
        ...
        if (data && data.buyer) {
            ...
            const result = await multichainRpc("publish", [SettlementStream, key, { "json": data }]);
...
router.get('/get_settlement_data', async (req, res) => {
    try {
        const settlement_stream = process.env.SETTLEMENT_STREAM;

        const { user_code } = req.query; // Get user_code from query params

        // Fetching stream data from Multichain
        const result = await multichainRpc('liststreamitems', [settlement_stream]);
