...
const pushTradeData = async (TradeStream, rawData) => {
    try {
        if (!rawData) {
        ...
        if (data && data.buyer) {
            ...
            const result = await multichainRpc("publish", [TradeStream, key, { "json": data }]);
...
router.get('/get_trade_data', async (req, res) => {
    try {
        const trade_stream = process.env.TRADES_STREAM;

        const { user_code } = req.query; // Get user_code from query params

        // Fetching stream data from Multichain
        const result = await multichainRpc('liststreamitems', [trade_stream]);
